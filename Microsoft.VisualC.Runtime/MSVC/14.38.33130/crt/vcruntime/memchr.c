        ...
        )
{
    ...
    if ((uintptr_t)buf & 0x7) {
        // If buf is not 8-byte aligned, we should not start reading 8 bytes from buf, because if buf
        // crosses a page boundary, we may land on an unmapped page. The C11/C++17 standard says that
        // "this function behaves as if it reads the characters sequentially and stops as soon as a
        // matching character is found." Therefore, if there is a match before touching the unmapped page,
        // the program should not crash but return successfully. To make sure we are conformant to the
        // standard, when the input buffer is not 8-byte aligned, we align the address first, and later
        // ignore the bytes before the beginning of the buffer.

        chbuf = (unsigned char *)buf;
        shift_count = ((uintptr_t)buf & 0x7);
        cnt += shift_count;
        buf = chbuf - shift_count;
    }
    ...
    // If the input buffer is not 8-byte aligned, we need to clear the masks
    // from the aligned address until the beginning of the buffer.

    ch_lword0 <<= (shift_count << 3);
    ...
    if (ch_bitoffset != 64) {
        end = (char *)((ch_bitoffset >> 3) + shift_count + (intptr_t)(src_a64));
        return ((uintptr_t)end - (uintptr_t)buf < cnt) ? end : NULL;
    }
