...
// This is used as the initializer if a category does not need initialization.
    ...
    ****/
    ...
    )
    {
        ...
        if (__acrt_MultiByteToWideChar(CP_ACP, 0, _locale, -1, _wlocale, _countof(_wlocale)) == 0) { // conversion to wide char failed
            return nullptr;
        }
    ...
    */
                ...
                if ((_wlocale[0] == L'L') && (_wlocale[1] == L'C') && (_wlocale[2] == L'_')) {
                    ...
                    if (retval != 0) {
                        for (i = LC_MIN; i <= LC_MAX; i++) {
                            if (i != LC_ALL) {
                                if (wcscmp(lctemp, ploci->lc_category[i].wlocale) != 0) { // does not match the LC_ALL locale
                                    if (_wsetlocale_set_cat(ploci, i, lctemp)) {
                                        fLocaleSet++;   /* record a success */
                                    }
                                    else {
    ...
    static int get_default_code_page(wchar_t const* const valid_windows_locale_name)
    {
        ...
        if (size_written == 0 || code_page == 0) {   // Default to UTF-8 if could not find or no default code page.
            return CP_UTF8;
        }
