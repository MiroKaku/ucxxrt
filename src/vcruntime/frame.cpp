...
#include "vcruntime/ehassert.h"    // This project's versions of standard assert macros
#include "vcruntime/ehdata.h"      // Declarations of all types used for EH
#include "vcruntime/ehdata4.h"
#include "vcruntime/ehhooks.h"     // Declarations of hook variables and callbacks
#include "vcruntime/trnsctrl.h"  // Routines to handle transfer of control (trnsctrl.asm)

#include "vcruntime/ehhelpers.h"

#if __has_include(<Unknown.h>)
#include <Unknown.h>
#else
#include <Unknwn.h>
#endif

#include "shared/ucxxrt_traits.h"
...
#define cxxReThrow   (RENAME_BASE_PTD(__ucxxrt_getptd)()->_cxxReThrow)
...
//
#define _pCurrentFuncInfo       (*((ESTypeList **)&(RENAME_BASE_PTD(__ucxxrt_getptd)()->_curexcspec)))

// In the satellite build, __ucxxrt_getptd uses satellite's PTD
// In the non-DLL build, __ucxxrt_getptd uses the main PTD which was updated to have this field
#define CatchStateInParent      (__ucxxrt_getptd()->_CatchStateInParent)
    ...
    */
    ...
    static void FindHandler(
        ...
        if (PER_IS_MSVC_EH(pExcept)) {
            ...
            if constexpr (traits::$$is_same<T, RENAME_EH_EXTERN(__FrameHandler4)>)
            {
...
#error Not yet implemented.
    ...
    // through RtlUnwind (STATUS_CONSULIDATE_FRAMES) mechanism.
        ...
        static_assert(!traits::$$is_same<T, RENAME_EH_EXTERN(__FrameHandler4)>, "Not yet implemented.");
        if (pMarkerRN == nullptr) {
            RENAME_EH_EXTERN(_UnwindNestedFrames)(pRN, pExcept);
        }
        else {
    ...
    {
        RtlRaiseException((EXCEPTION_RECORD*)pThisException);
    }
